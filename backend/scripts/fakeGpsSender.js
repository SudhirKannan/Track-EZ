...
// 👇 These 2 lines simulate __dirname in ESM
...
if (!JWT_TOKEN) {
    console.error('❌ JWT_TOKEN is missing. Add it to backend/.env');
    process.exit(1);
}

// Predefined route coordinates (Chennai)
const routes = [
    {
        coordinates: [
            { lat: 13.0827, lng: 80.2707 }, // Chennai Central
...
class FakeGPSSender {
    ...
    async initialize() {
        try {
            ...
            // 🔐 GET /buses with Authorization header
            ...
            this.buses = data.buses.map((bus) => bus._id); 

            if (!this.buses.length) {
                throw new Error('No buses found in database.');
            }
